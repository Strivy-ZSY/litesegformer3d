...
我们基于SegFormer3D设计了轻量高效的医学图像分割网络 **LiteSegFormer3D**，在提升分割精度的同时，训练时间在三个数据集上缩短了1/3到1/2。通过引入新型前馈处理方式加速训练，采用高斯核函数的注意力机制减少参数规模并增强空间纹理拟合，结合非对称卷积弥补浅层特征和金字塔式监督加速收敛，实现了高效且精确的分割算法。

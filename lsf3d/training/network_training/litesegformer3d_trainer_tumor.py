...
from lsf3d.training.network_training.Trainer_tumor import Trainer_tumor
...
class litesegformer3d_trainer_tumor(Trainer_tumor):
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
        ...
        import numpy as np
        import os
        output_dir = join(self.output_folder, 'validation_raw')
        
        for fname in os.listdir(output_dir):
            if fname.endswith('.nii.gz'):
                filepath = join(output_dir, fname)
                img = nib.load(filepath)
                data = img.get_fdata()
                
                # 硬核三维翻转：左右(x)、前后(y)、上下(z)
                flipped_data = np.flip(data, axis=(0, 1))
                
                # 直接覆盖原文件
                nib.save(nib.Nifti1Image(flipped_data, img.affine), filepath)

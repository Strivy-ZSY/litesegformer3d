...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        # Store spatial shape before flattening
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, spatial_shape: Tuple[int, int, int] = None) -> torch.Tensor:
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, spatial_shape: Tuple[int, int, int] = None):
...
class MixVisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        B, N, C = x.shape
        spatial_shape1 = (D1, H1, W1)
        
        for i, blk in enumerate(self.tf_block1):
            x = blk(x, spatial_shape1)
        x = self.norm1(x)
        ...
        B, N, C = x.shape
        spatial_shape2 = (D2, H2, W2)
        
        for i, blk in enumerate(self.tf_block2):
            x = blk(x, spatial_shape2)
        x = self.norm2(x)
        # (B, N, C) -> (B, D, H, W, C) -> (B, C, D, H, W)
        ...
        B, N, C = x.shape
        spatial_shape3 = (D3, H3, W3)
        
        for i, blk in enumerate(self.tf_block3):
            x = blk(x, spatial_shape3)
        x = self.norm3(x)
        # (B, N, C) -> (B, D, H, W, C) -> (B, C, D, H, W)
        ...
        B, N, C = x.shape
        spatial_shape4 = (D4, H4, W4)
        
        for i, blk in enumerate(self.tf_block4):
            x = blk(x, spatial_shape4)
        x = self.norm4(x)
        # (B, N, C) -> (B, D, H, W, C) -> (B, C, D, H, W)
...
def cube_root(n):
    ...
    if isinstance(n, torch.Tensor):
        return int(round(n.item() ** (1.0 / 3.0)))

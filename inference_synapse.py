...
def align_shapes(pred, label):
    if pred.shape == label.shape:
        return pred, label

    min_shape = [min(pred.shape[i], label.shape[i]) for i in range(3)]
...
def dice(pred, label):
    pred, label = align_shapes(pred, label)
...
def test(fold): 
    label_path= "lsf3d/inferencedata/labsynapse"
    infer_path = "lsf3d/inferencedata/anssynapse" # Replace with your generated label file
    ...
    if not os.path.exists(file):
        os.makedirs(file)
    fw = open(file+'/dice_pre_synapse.txt', 'a')
